...
def process_pdf(
    pdf_path: Path, parser: str = "marker", output_dir: Path = None, cache: bool = True
):
    ...
    try:
        ...
        output_path = (
            str(doc_output_dir / f"{pdf_name}_{parser}.md") if doc_output_dir else None
        )
        ...
        logger.info(
            f"{parser} parsing complete for {pdf_path.name}: {parser_time:.2f}s, text length: {len(parsing_result.get('text', ''))} chars"
        )

    except Exception as e:
        logger.error(f"ERROR processing {pdf_path.name}: {str(e)}", exc_info=True)
        result.update({"extraction_success": False, "error": str(e)})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Parser to use for PDF processing",
    )
    parser.add_argument(
