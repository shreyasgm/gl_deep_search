...
def parse_marker_preset(
    ...
    if model not in model_configs:
        raise ValueError(
            f"Unknown model '{model}'. Available models: {list(model_configs.keys())}"
        )
    ...
    if output_path is None:
        ext = (
            ".md" if config.get("output_format", "markdown") == "markdown" else ".json"
        )
    ...
    if config.get("use_llm"):
        ...
        if "openai" in llm_service.lower():
            ...
            if not api_key:
                raise OSError("OPENAI_API_KEY required for OpenAI service")
            ...
            if not api_key:
                raise OSError(
                    "GOOGLE_API_KEY or GEMINI_API_KEY required for Gemini service"
                )
            ...
            if not api_key:
                raise OSError("CLAUDE_API_KEY required for Claude service")
