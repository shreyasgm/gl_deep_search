...
def expand_publications_to_file_level(input_csv: Path, output_csv: Path) -> None:
    ...
    # Explode so that each row has one file_url, duplicating the other columns
    df = df.explode("file_urls")

    # Rename to file_url for clarity
    df = df.rename(columns={"file_urls": "file_url"})

    # Handle rows with missing file_url (e.g., empty original field)
    df["missing_fileurl"] = df["file_url"].isna()

    # Clean/standardize file_url column
    df["file_url"] = df["file_url"].astype(str).str.strip().replace("nan", None)

    # Generate additional file-level information
    def build_row(row):
        url = row["file_url"]
        pub_id = row["paper_id"]
    ...
    # Apply row-wise and join results
...
if __name__ == "__main__":
    ...
    expand_publications_to_file_level(input_csv=Path(args.input), output_csv=Path(args.output))